...
| [MuJoCo application container](mujoco/rocky9-cpu/) | Installing MuJoCo from wheels on Rocky Linux 9, with headless offscreen rendering through EGL | You want to run physics simulation workloads from a container |

The worker-equivalent image is useful for local compatibility work and package builds. The Blender, MoonRay, and MuJoCo images are application-container examples; Blender includes its own deployment resources and instructions, MoonRay builds the renderer from source, and MuJoCo installs from published wheels.
