...
"
        echo "Simple Plugins: Found addon package: $_name"
    done
    for _entry in "$_SP_ADDONS_DIR"/*.py; do
        ...
        _SP_ADDON_LIST="${_SP_ADDON_LIST}    \"${_name}\",
"
        echo "Simple Plugins: Found addon script: $_name"
    done

    if [ -n "$_SP_ADDON_LIST" ]; then
        cat > "$_SP_STARTUP_DIR/simple_plugins_auto_enable.py" << PYEOF
"""Simple Plugins: Auto-enable addons downloaded from S3.

This script is generated by the 999-blender-simple-plugins activate script
...
_auto_enable()
PYEOF
        echo "Simple Plugins: Generated auto-enable startup script for $(echo "$_SP_ADDON_LIST" | grep -c '\"') addon(s)"
    fi
